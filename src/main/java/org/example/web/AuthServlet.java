...
public class AuthServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        switch (request.getServletPath()) {
            ...
            case "/logout" -> { request.getSession().invalidate(); response.sendRedirect(request.getContextPath() + "/login.html"); }
    ...
    private void signup(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        if (name.isBlank() || !email.contains("@") || password == null || password.length() < 8) {
            response.sendRedirect(request.getContextPath() + "/signup.html?error=validation"); return;
        }
        try (Session session = HibernateUtil.getSessionFactory().openSession()) {
            if (session.createQuery("from User where email = :email", User.class).setParameter("email", email).uniqueResult() != null) {
                response.sendRedirect(request.getContextPath() + "/signup.html?error=exists"); return;
            }
    ...
    private void login(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        try (Session session = HibernateUtil.getSessionFactory().openSession()) {
            ...
            if (user == null || password == null || !PasswordUtil.matches(password, user.getPasswordHash())) {
                response.sendRedirect(request.getContextPath() + "/login.html?error=invalid"); return;
            }
